...
The desktop smoke test starts FileInfo and reads the generated mzML. The next step then starts every registered tool, replays all upstream TOPP tests against the installation, and records the bundled OpenSSL, zlib, curl, SQLite and Qt versions (see [What every run asserts](README.md#what-every-run-asserts)). Native loader logs use LD_DEBUG on Linux and DYLD_PRINT_LIBRARIES on macOS; hardened macOS executables can suppress that trace.
